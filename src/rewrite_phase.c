...
gint g_record_descmp_by_length(struct containerRecord* a,
	...
	return (b->chunk_size + b->delta_size) - (a->chunk_size + a->delta_size);
}
...
struct chunk* rewrite_buffer_pop() {
	...
	if (c && !CHECK_CHUNK(c, CHUNK_FILE_START)
			&& !CHECK_CHUNK(c, CHUNK_FILE_END)
			&& !CHECK_CHUNK(c, CHUNK_SEGMENT_START)
			&& !CHECK_CHUNK(c, CHUNK_SEGMENT_END)) {
		/* A normal chunk */
...
void start_rewrite_phase() {
    rewrite_queue = sync_queue_new(2000);
