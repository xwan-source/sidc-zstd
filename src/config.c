...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		if (strcasecmp(argv[0], "working-directory") == 0 && argc == 2) {
            ...
            if (destor.chunk_algorithm != CHUNK_AE) {
			    int pwr;
		...
		} else if (strcasecmp(argv[0], "har-include-self-reference") == 0
				&& argc == 2) {
			destor.har_include_self_reference = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "rewrite-chunk-referencing-delta") == 0
				&& argc == 2) {
			destor.rewrite_chunk_referencing_delta = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "rewrite-har-utilization-threshold") == 0
				&& argc == 2) {
			destor.rewrite_har_utilization_threshold = atof(argv[1]);
		} else if (strcasecmp(argv[0], "base-container-prefetching-percent") == 0
				&& argc == 2) {
			destor.base_container_prefetching_percent = atof(argv[1]);
		} else if (strcasecmp(argv[0], "sparse-chunk-number-threshold") == 0
				&& argc == 2) {
			destor.sparse_chunk_number_threshold = atof(argv[1]);
		} else if (strcasecmp(argv[0], "rewrite-har-rewrite-limit") == 0
				&& argc == 2) {
			destor.rewrite_har_rewrite_limit = atof(argv[1]);
		} else if (strcasecmp(argv[0], "dense-utilization-threshold") == 0
				&& argc == 2) {
			destor.dense_utilization_threshold = atof(argv[1]);
		} else if (strcasecmp(argv[0], "sparse-utilization-threshold") == 0
				&& argc == 2) {
			destor.sparse_utilization_threshold = atof(argv[1]);
		} else if (strcasecmp(argv[0], "enable-full-backup-after-threshold") == 0
				&& argc == 2) {
			destor.enable_full_backup_after_threshold = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "full-backup-threshold-of-rewritten") == 0
				&& argc == 2) {
			destor.full_backup_threshold_of_rewritten = atof(argv[1]);
		} else if (strcasecmp(argv[0], "restore-speed-factor-threshold") == 0
				&& argc == 2) {
			destor.restore_speed_factor_to_fullbackup = atof(argv[1]);
		} else if (strcasecmp(argv[0], "enable-dedup-in-dense-full-backup") == 0
				&& argc == 2) {
			destor.enable_dedup_in_dense_full_backup = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "clear-history-info-for-full-backup") == 0
				&& argc == 2) {
			destor.clear_history_info_for_full_backup = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "rewrite-enable-cache-aware") == 0
				&& argc == 2) {
			destor.rewrite_enable_cache_aware = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "base-container-num") == 0
				&& argc == 2) {
			destor.base_container_num = atoi(argv[1]);
		} else if (strcasecmp(argv[0], "sketch-method") == 0
				&& argc == 2) {
			if (strcasecmp(argv[1], "SF") == 0)
                destor.sketch_method = SUPER_FEATURE;
            else if (strcasecmp(argv[1], "Finesse") == 0)
                destor.sketch_method = FINESSE;
			else if (strcasecmp(argv[1], "Odess") == 0)
                destor.sketch_method = ODESS;
		} else if (strcasecmp(argv[0], "mini-sketch-size") == 0
                && argc == 2) {
            destor.mini_sketch_size = atoi(argv[1]);
        } else if (strcasecmp(argv[0], "features-per-sf") == 0
                && argc == 2) {
            destor.features_per_sf = atoi(argv[1]);
        } else if (strcasecmp(argv[0], "base-container-prefetch-num") == 0
                && argc == 2) {
            destor.base_container_prefetch_num = atoi(argv[1]);
        } else if (strcasecmp(argv[0], "hashing-method") == 0
                && argc == 2) {
            if (strcasecmp(argv[1], "sha1") == 0)
                destor.hashing_method = SHA_1;
            else if (strcasecmp(argv[1], "sha256") == 0)
                destor.hashing_method = SHA_256;
        } else if (strcasecmp(argv[0], "remove-rewritten-sparse-chunks") == 0
                && argc == 2) {
            destor.remove_rewritten_sparse_chunks = yesnotoi(argv[1]);
		...
		} else if (strcasecmp(argv[0], "simulated-restore-cache-size") == 0
				&& argc == 2) {
			destor.simulated_restore_cache_size = atoi(argv[1]);
		} else if (strcasecmp(argv[0], "enable-locality-based-approach-for-resembling") == 0
				&& argc == 2) {
			destor.enable_locality_based_approach_for_resembling = yesnotoi(argv[1]);
		} else if (strcasecmp(argv[0], "number-of-entries-for-caf") == 0
				&& argc == 2) {
			destor.number_of_item_for_caf = atoi(argv[1]);
...
void load_config() {
	...
	//if (destor.hashing_method == SHA_256)
