 ...
 * The main drawback of capping is that capping overlook the relationship
 * of the consecutive buffers.
 */
void *cap_rewrite(void* arg) {
	...
	while (1) {
        ...
        /* if rewrite buffer is full, calculate the top containers and pop all chunks in buffer */
		cap_segment_get_top();
		...
		while ((c = rewrite_buffer_pop())) {
			if (!CHECK_CHUNK(c,	CHUNK_FILE_START)
                ...
                if(CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
				    if (!g_hash_table_contains(top, &c->id)
							&& !(dense_container_check(c->id))) {
					    /* not in TOP and not in dense container lists */
					...
					if (!CHECK_CHUNK(c, CHUNK_OUT_OF_ORDER)
						...
						assert(c->base_id = -1);
						if ((g_hash_table_lookup(top, &c->base_id) == NULL)
							&& (!dense_container_check(c->base_id)))
